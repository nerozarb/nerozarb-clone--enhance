...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-[1200px]">
      <div className="rounded-full px-8 backdrop-blur-xl bg-gradient-to-br from-white/15 via-white/8 to-black/20 border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.15),inset_0_-1px_0_rgba(0,0,0,0.2)] relative overflow-hidden !w-[1135px] !h-full">
        ...
        {/* Subtle shine effect */}
        <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/3 to-transparent rounded-full pointer-events-none"></div>
        ...
        <div className="flex items-center justify-between h-20 relative z-10">
          <Link href="/" className="flex items-center space-x-3 group">
            <div className="relative">
              <Image
                ...
                width={48}
                height={48}
                className="h-12 w-12 transition-all duration-500 group-hover:scale-110 group-hover:drop-shadow-[0_0_20px_rgba(255,255,255,0.6)]" />

            </div>
            <span className="text-xl font-bold tracking-tight text-white drop-shadow-[0_2px_8px_rgba(255,255,255,0.3)]">NEROZARB</span>
          </Link>
          ...
          <div className="lg:hidden">
            <button
              ...
              className="relative w-10 h-10 flex flex-col items-center justify-center space-y-1 group"
      ...
      {isMenuOpen &&
      <div className="lg:hidden mt-2 rounded-3xl backdrop-blur-xl bg-gradient-to-br from-white/20 via-white/10 to-white/5 border border-white/30 shadow-[0_8px_32px_rgba(255,255,255,0.1)] animate-in slide-in-from-top-4 duration-300 overflow-hidden relative">
          ...
          <div className="px-8 py-4 flex flex-col space-y-4 relative z-10">
            {navLinks.map((link) =>
          <Link
            key={link.href}
            href={link.href}
            className="block py-2 text-base font-medium text-white/90 hover:text-white transition-colors drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)]"
            onClick={() => setIsMenuOpen(false)}>

                {link.label}
              </Link>
          )}
            <div className="border-t border-white/20 pt-4 flex flex-col items-start space-y-4">
              <button
              ...
              <Link href="/contact" className="w-full" onClick={() => setIsMenuOpen(false)}>
                <button className="w-full h-10 px-6 bg-white text-black font-semibold rounded-full hover:scale-105 hover:shadow-[0_4px_20px_rgba(255,255,255,0.4)] transition-all duration-300">
                  Contact
                </button>
              </Link>
            </div>
